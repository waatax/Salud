...
export const KnowledgePage: React.FC<Props> = ({ page, onNavigatePage }) => {
  ...
  return (
    <article className="space-y-8 font-sans text-xs pb-16 max-w-4xl mx-auto">
      ...
      <header className="space-y-4 border-b border-salud-dark-border/80 pb-6">
        <div className="flex flex-wrap items-center justify-between gap-3 font-mono text-[11px] text-slate-400">
          <div className="flex items-center gap-2">
            <span className="text-salud-amber font-bold">{page.id}</span>
            <span>·</span>
        ...
        <h1 className="text-2xl sm:text-3xl font-display font-extrabold text-salud-dark-text dark:text-salud-dark-text light:text-salud-light-text tracking-tight">
          {page.title_zh}
        </h1>
        <div className="text-sm font-mono text-salud-dark-muted dark:text-salud-dark-muted light:text-salud-light-muted">
          {page.title_en}
        </div>
      </header>

      {/* ── 00 Hook (Spec §3.4) ── */}
      <section className="p-4 sm:p-5 rounded-2xl border border-salud-amber-500/30 bg-gradient-to-r from-salud-amber-500/10 via-salud-dark-card/40 to-transparent">
        <div className="flex items-start gap-3">
          <Sparkles className="w-5 h-5 text-salud-amber-400 shrink-0 mt-0.5" />
          <div className="space-y-1">
            <span className="text-[10px] font-mono uppercase tracking-wider text-salud-amber-400 font-bold block">
              00 Hook · 機制核心提問
            </span>
            <p className="text-sm sm:text-base font-semibold text-salud-dark-text dark:text-salud-dark-text light:text-salud-light-text leading-relaxed">
              {page.hook}
            </p>
          </div>
        </div>
      ...
      <section className="space-y-3">
        <div className="flex items-center justify-between border-b border-salud-dark-border/60 pb-2">
        ...
        <div className="space-y-2.5">
          {page.kps.map((kp) => {
            ...
            return (
              <div
                key={kp.id}
                className="rounded-xl border border-salud-dark-border/80 bg-salud-dark-surface dark:bg-salud-dark-surface light:bg-salud-light-surface overflow-hidden transition-all duration-200"
              >
                {/* KP Summary Bar */}
                <button
                  onClick={() => toggleKP(kp.id)}
                  className="w-full p-3.5 text-left flex items-start justify-between gap-3 hover:bg-salud-dark-card/40 transition-colors"
                >
                  <div className="space-y-1 flex-1">
                    <div className="flex items-center gap-2 font-mono text-[10px]">
                      <span className="px-1.5 py-0.5 rounded bg-slate-800 text-salud-amber-400 font-bold border border-slate-700">
                        {kp.id}
                      </span>
                ...
                {isExpanded && (
                  <div className="px-4 pb-4 pt-1 space-y-3 border-t border-salud-dark-border/40 text-xs text-slate-300 animate-fade-in">
                    <p className="leading-relaxed text-slate-200 font-sans text-xs sm:text-sm">
                      {kp.statement}
                    </p>

                    <div className="p-3 rounded-lg bg-salud-dark-card/60 border border-slate-800 space-y-1">
                      <span className="text-[11px] font-mono text-salud-amber-400 font-semibold block">
                    ...
                    {kp.common_misconception && (
                      <div className="p-2.5 rounded-lg bg-red-950/20 border border-red-500/30 text-red-200 text-xs">
                        <strong className="font-mono text-red-300">常見誤解：</strong>
                        <span>{kp.common_misconception}</span>
                      </div>
                    )}

                    {/* Metadata tags */}
                    <div className="flex flex-wrap items-center justify-between text-[10px] font-mono text-slate-400 pt-1 border-t border-slate-800/80 gap-2">
      ...
      <section className="p-4 sm:p-5 rounded-2xl border border-salud-dark-border bg-slate-900/60 space-y-3">
        <div className="flex items-center gap-2 text-salud-amber-400 font-display font-bold text-sm sm:text-base">
      ...
      <section className="space-y-3">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
          ...
          <div className="p-3.5 rounded-xl border border-salud-cyan/40 bg-salud-cyan-950/20 space-y-1.5">
            <span className="text-[10px] font-mono uppercase tracking-wider text-salud-cyan-400 font-bold block">
              Tier 2 · 需主動規劃留意
            </span>
          ...
          <div className="p-3.5 rounded-xl border border-salud-amber/40 bg-salud-amber-950/20 space-y-1.5">
            <span className="text-[10px] font-mono uppercase tracking-wider text-salud-amber-400 font-bold block">
              Tier 3 · 需專業個別化指導
            </span>
      ...
      {page.not_for_you && page.not_for_you.length > 0 && (
        <section className="p-4 rounded-xl border border-slate-800 bg-slate-900/60 space-y-2">
          <h4 className="font-bold text-xs font-mono text-slate-300 uppercase tracking-wider">
      ...
      <footer className="p-4 sm:p-5 rounded-2xl border border-salud-dark-border bg-salud-dark-card/30 space-y-3 font-mono text-[11px] text-slate-400">
        <div className="flex flex-wrap items-center justify-between gap-2 border-b border-salud-dark-border/40 pb-2">
          <span className="font-bold text-slate-300">11 實證架構與治理紀錄</span>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-2">
          <div>
            ...
            <span className="text-slate-300 font-bold">{page.last_reviewed}</span>
          </div>
          <div>
        ...
        <div className="pt-2 border-t border-salud-dark-border/40 text-[10px] text-slate-500 leading-relaxed">
