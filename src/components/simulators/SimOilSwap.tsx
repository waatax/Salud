...
export const SimOilSwap: React.FC = () => {
  ...
  return (
    <div className="w-full max-w-3xl rounded-2xl border border-salud-amber/30 bg-salud-dark-surface dark:bg-salud-dark-surface light:bg-salud-light-surface overflow-hidden shadow-2xl font-sans text-xs">
      {/* Header */}
      <div className="p-4 sm:p-5 border-b border-salud-dark-border bg-gradient-to-r from-salud-amber-950/40 via-salud-dark-card/60 to-transparent flex flex-wrap items-center justify-between gap-3">
        <div className="flex items-center gap-2.5">
          <div className="p-2 rounded-xl bg-salud-amber/20 border border-salud-amber/40 text-salud-amber-400">
            <RefreshCw className="w-5 h-5" />
          </div>
          <div>
            <div className="flex items-center gap-2">
              <span className="font-mono text-[10px] uppercase tracking-wider px-2 py-0.5 rounded bg-salud-amber/20 text-salud-amber-300 border border-salud-amber/40">
                SIM-OIL-SWAP · v0.1
              </span>
              <span className="text-[11px] text-slate-400 font-mono">Chapter O 專屬模擬器</span>
            </div>
        ...
        <div className="flex items-center gap-2">
          ...
          >
            <Table className="w-3.5 h-3.5 text-salud-amber" />
      ...
      {/* Selector Toolbar */}
      <div className="p-4 sm:p-6 grid grid-cols-1 md:grid-cols-3 gap-4 border-b border-salud-dark-border/80 bg-salud-dark-card/20">
        {/* Swap From */}
        <div>
          ...
          <select
            ...
            className="w-full p-2 rounded-lg bg-slate-800 border border-slate-700 text-slate-100 text-xs font-medium focus:border-salud-amber focus:outline-none"
          >
            {EDIBLE_OILS.map((o) => (
        ...
        {/* Swap To */}
        <div>
          <label className="font-mono text-salud-cyan block mb-1 font-semibold">
            想替換的好油 (Swap To)：
          </label>
          <select
            value={toId}
            onChange={(e) => setToId(e.target.value)}
            className="w-full p-2 rounded-lg bg-slate-800 border border-salud-cyan/50 text-slate-100 text-xs font-medium focus:border-salud-cyan focus:outline-none"
          >
            {EDIBLE_OILS.map((o) => (
              <option key={o.id} value={o.id}>
                {o.name_zh} (SFA {o.sfa_pct}%, MUFA {o.mufa_pct}%)
        ...
        <div>
          <div className="flex justify-between font-mono text-slate-300 mb-1">
            <span>每日用量：</span>
            <strong className="text-salud-amber-300">
      ...
      <div className="px-4 sm:px-6 py-3 border-b border-salud-dark-border/60 flex flex-wrap items-center justify-between gap-3 text-xs">
        <div className="flex items-center gap-2">
          <span className="font-mono text-slate-400">主要烹飪場景：</span>
              ...
              <button
                ...
                className={`px-2.5 py-1 rounded-md font-medium transition-all ${
                  cookingContext === ctx.id
                    ? 'bg-slate-700 text-salud-amber-300 border border-salud-amber/40 font-bold'
        ...
        <label className="flex items-center gap-1.5 cursor-pointer font-mono text-slate-400">
          <input
            type="checkbox"
            checked={isOnStatin}
            onChange={(e) => setIsOnStatin(e.target.checked)}
            className="rounded border-slate-700 text-salud-cyan focus:ring-salud-cyan"
          />
      ...
      {isFlaxseedHeated && (
        <div className="mx-4 sm:mx-6 my-4 p-3 rounded-xl border border-red-500/60 bg-red-950/30 text-red-200 flex items-start gap-2">
      ...
      {isOnStatin && (
        <div className="mx-4 sm:mx-6 my-2 p-3 rounded-xl border border-salud-cyan/40 bg-salud-cyan-950/20 text-salud-cyan-200 flex items-center gap-2">
          <ShieldCheck className="w-4 h-4 text-salud-cyan-400 shrink-0" />
          <span>
      ...
      <div className="p-4 sm:p-6 space-y-4">
        {showTable ? (
            ...
            <table className="w-full text-left font-mono text-xs">
              ...
              <tbody className="divide-y divide-slate-800 text-slate-300">
                <tr>
                  ...
                  <td className={`py-2 font-bold ${sfaDeltaG <= 0 ? 'text-emerald-400' : 'text-red-400'}`}>
                ...
                <tr>
                  ...
                  <td className={`py-2 font-bold ${mufaDeltaG >= 0 ? 'text-salud-cyan' : 'text-slate-400'}`}>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
            ...
            <div className={`p-4 rounded-xl border text-center ${
              sfaDeltaG <= 0 ? 'border-emerald-500/40 bg-emerald-950/20' : 'border-red-500/40 bg-red-950/20'
            }`}>
              <span className="text-[11px] font-mono text-slate-400 block">飽和脂肪變化 (SFA)</span>
              <strong className={`text-2xl font-display block my-1 ${
                sfaDeltaG <= 0 ? 'text-emerald-300' : 'text-red-300'
              }`}>
                {sfaDeltaG > 0 ? `+${sfaDeltaG}` : sfaDeltaG} g/日
              </strong>
            ...
            {/* MUFA Metric */}
            <div className="p-4 rounded-xl border border-salud-cyan/40 bg-salud-cyan-950/20 text-center">
              <span className="text-[11px] font-mono text-slate-400 block">單元不飽和變化 (MUFA)</span>
              <strong className="text-2xl font-display text-salud-cyan-300 block my-1">
                {mufaDeltaG > 0 ? `+${mufaDeltaG}` : mufaDeltaG} g/日
              </strong>
              <span className="text-[11px] font-mono text-slate-400">
            ...
            <div className="p-4 rounded-xl border border-slate-800 bg-slate-900/60 text-center">
              <span className="text-[11px] font-mono text-slate-400 block">多元不飽和變化 (PUFA)</span>
              <strong className="text-2xl font-display text-slate-200 block my-1">
                {pufaDeltaG > 0 ? `+${pufaDeltaG}` : pufaDeltaG} g/日
              </strong>
              <span className="text-[11px] font-mono text-slate-400">
        ...
        <div className="p-4 rounded-xl bg-slate-900/90 border border-slate-800 space-y-2">
          <div className="flex justify-between items-center text-[11px] font-mono text-slate-300">
            ...
            <span className="text-salud-amber-400">
              換油後佔比估算：{sfaToPctE}%E
            </span>
          </div>
          ...
          <div className="flex justify-between text-[10px] font-mono text-slate-500">
            <span>0%E</span>
            <span className="text-red-400 font-bold">10%E (WHO 警戒線)</span>
            <span>20%E</span>
          </div>
        </div>
        ...
        <div className="p-3.5 rounded-xl bg-slate-900/60 border border-slate-800 text-xs text-slate-300 flex items-start gap-2">
          <ArrowRight className="w-4 h-4 text-salud-amber-400 shrink-0 mt-0.5" />
          <div>
            <strong className="text-slate-100 font-semibold">烹調適用性評估：</strong>
            <span>
        ...
        <div className="pt-2 border-t border-slate-800 text-[11px] font-mono text-slate-500 leading-relaxed">
