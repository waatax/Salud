...
export const InfographLongevityCompounds: React.FC = () => {
  ...
  return (
    <div className="space-y-6">
      {/* Header Banner */}
      <div className="p-5 rounded-2xl bg-gradient-to-r from-slate-900 via-amber-950/40 to-slate-900 border border-amber-500/30 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
        ...
        <div className="px-3 py-1.5 rounded-xl bg-amber-500/10 border border-amber-500/30 text-amber-300 text-xs font-mono font-bold shrink-0">
          7 款代表性長壽化合物
        </div>
      </div>
          ...
          <button
            ...
            className={`p-3 rounded-2xl border text-left transition-all ${
              selectedCompoundId === c.id
                ? 'border-amber-400 bg-amber-500/20 text-white shadow-lg ring-2 ring-amber-400/20 scale-105'
      ...
      <div className="rounded-3xl border border-amber-500/30 bg-slate-900/90 p-6 sm:p-8 space-y-6 shadow-2xl relative overflow-hidden">
        {/* Header */}
        <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-4 border-b border-slate-800 pb-5">
          <div className="space-y-1.5">
            <div className="flex items-center gap-2 flex-wrap text-xs font-mono">
              ...
              <span className="text-slate-400">主審專席: {selectedCompound.lead_reviewer_id}</span>
            </div>
          ...
          <div className="p-3.5 rounded-2xl bg-amber-500/10 border border-amber-500/30 text-center shrink-0">
            <div className="text-[10px] font-mono text-slate-400">ITP 最大壽命延長效益</div>
        ...
        <div className="p-4 sm:p-5 rounded-2xl bg-slate-950 border border-slate-800 space-y-3">
          ...
          <div className="space-y-2 text-xs">
            {[
              { label: '人體臨床試驗實證 (Human Evidence)', score: selectedCompound.radar_scores.human_evidence, color: 'bg-blue-400' },
              { label: '哺乳類壽命延長潛力 (Lifespan Potential)', score: selectedCompound.radar_scores.lifespan_potential, color: 'bg-amber-400' },
              { label: '長期用藥安全耐受度 (Safety Profile)', score: selectedCompound.radar_scores.safety_profile, color: 'bg-emerald-400' },
              { label: '獲取便利度與成本 (Accessibility)', score: selectedCompound.radar_scores.accessibility, color: 'bg-purple-400' },
              { label: '器官衰老生物標記影響 (Biomarker Impact)', score: selectedCompound.radar_scores.biomarker_impact, color: 'bg-salud-cyan' },
            ].map((item, idx) => (
