...
export const InfographHormesisProtocol: React.FC = () => {
  ...
  return (
    <div className="space-y-8">
      {/* Header Banner */}
      <div className="p-5 rounded-2xl bg-gradient-to-r from-slate-900 via-teal-950/40 to-slate-900 border border-teal-500/30 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
      ...
      <div className="rounded-3xl border border-teal-500/30 bg-slate-900/80 p-6 sm:p-8 space-y-4">
        <div className="flex items-center justify-between border-b border-slate-800 pb-3">
          <div className="text-sm font-bold text-white flex items-center gap-2">
            <Activity className="w-4 h-4 text-teal-400" />
            <span>激效適應生理 U 型劑量曲線 (Hormetic Dose-Response)</span>
          </div>
          <span className="text-[10px] font-mono text-slate-400">EC-39 專家審定</span>
        </div>

        <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 text-xs">
          <div className="p-4 rounded-2xl bg-slate-950/70 border border-slate-800 space-y-1.5">
            <div className="font-bold text-slate-400">1. 欠缺微壓力 (過度舒適)</div>
            <div className="text-[11px] text-slate-500 font-mono">恆溫空調 · 隨手高糖 · 缺乏運動</div>
            <p className="text-[11px] text-slate-400 leading-relaxed pt-1 border-t border-slate-850">
              熱休克蛋白與抗氧化酵素沉睡，自噬清運機制停滯，細胞抗逆修復能力逐漸萎縮退化。
            </p>
          </div>

          <div className="p-4 rounded-2xl bg-teal-950/30 border border-teal-500/40 space-y-1.5 ring-2 ring-teal-500/20">
            <div className="font-bold text-teal-300">2. 黃金激效區 (Hormetic Zone)</div>
            <div className="text-[11px] text-teal-400 font-mono">芬蘭桑拿 · 冷水浴 · 16/8 斷食 · Zone 2</div>
            <p className="text-[11px] text-slate-300 leading-relaxed pt-1 border-t border-teal-500/30">
              活化 Hsp70、RBM3、PGC-1$\alpha$、SIRT1 與 AMPK，誘導損傷自噬與粒線體新生，死亡風險顯著降低！
            </p>
          </div>

          <div className="p-4 rounded-2xl bg-slate-950/70 border border-slate-800 space-y-1.5">
            <div className="font-bold text-rose-400">3. 毒性過負荷 (Exhaustion)</div>
            <div className="text-[11px] text-rose-400/80 font-mono">超長時間蒸桑拿 · 深度低體溫 · 過度訓練</div>
            <p className="text-[11px] text-slate-400 leading-relaxed pt-1 border-t border-slate-850">
        ...
        {HORMESIS_PROTOCOLS.map((p) => (
          <button
            ...
            className={`p-3.5 rounded-2xl border text-left transition-all ${
              selectedModalityId === p.id
                ? 'bg-slate-800 border-teal-400 text-white shadow-lg ring-2 ring-teal-400/20'
                : 'bg-slate-900/60 border-slate-800 text-slate-400 hover:border-slate-700'
            }`}
          >
            <div className="text-[10px] font-mono text-teal-400 font-bold truncate">
      ...
      <div className="rounded-3xl border border-teal-500/30 bg-slate-900/90 p-6 sm:p-8 space-y-6 shadow-xl">
        <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-4 border-b border-slate-800 pb-5">
          <div className="space-y-1.5">
            <div className="flex items-center gap-2 text-xs font-mono">
              ...
              <span className="text-slate-400">主審專席: {selectedProtocol.lead_reviewer_id}</span>
            </div>
          ...
          <div className="flex items-center gap-2 font-mono text-xs">
            <div className="px-3 py-2 rounded-xl bg-slate-950 border border-slate-800 text-center">
              ...
              <div className="font-bold text-teal-400 mt-0.5">{selectedProtocol.frequency_per_week}</div>
            </div>
            <div className="px-3 py-2 rounded-xl bg-slate-950 border border-slate-800 text-center">
              <div className="text-[9px] text-slate-500">黃金有效劑量</div>
              <div className="font-bold text-amber-400 mt-0.5">{selectedProtocol.effective_dose}</div>
            </div>
          </div>
        </div>
      ...
      <div className="rounded-3xl border border-slate-800 bg-slate-900/60 p-6 sm:p-8 space-y-4">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-4 gap-3 pt-2">
          <div className="p-4 rounded-2xl bg-slate-950 border border-slate-800 space-y-1 text-xs">
            <div className="font-mono text-[10px] text-teal-400">07:00 ~ 08:30 早晨</div>
            <div className="font-bold text-white">晨間光照 + 2分鐘冷水</div>
          ...
          <div className="p-4 rounded-2xl bg-slate-950 border border-slate-800 space-y-1 text-xs">
            <div className="font-mono text-[10px] text-blue-400">12:00 ~ 18:00 午後</div>
            <div className="font-bold text-white">8小時進食窗 + Zone 2</div>
            <p className="text-[11px] text-slate-400 leading-relaxed">
              限制進食窗啟動代謝靈活性；Zone 2 有氧 45 分鐘最大化粒線體脂肪酸 $\beta$ 氧化。
            </p>
          </div>

          <div className="p-4 rounded-2xl bg-slate-950 border border-slate-800 space-y-1 text-xs">
            <div className="font-mono text-[10px] text-amber-400">18:30 ~ 20:00 傍晚</div>
            <div className="font-bold text-white">高溫桑拿 20 分鐘</div>
            <p className="text-[11px] text-slate-400 leading-relaxed">
          ...
          <div className="p-4 rounded-2xl bg-slate-950 border border-slate-800 space-y-1 text-xs">
            <div className="font-mono text-[10px] text-purple-400">22:00 ~ 06:30 深夜</div>
            <div className="font-bold text-white">膠淋巴系統全開 (SWS)</div>
            <p className="text-[11px] text-slate-400 leading-relaxed">
              深度慢波睡眠沖洗大腦 A$\beta$ 澱粉樣蛋白；禁食滿 12 小時啟動全身巨自噬流。
            </p>
          </div>
        </div>
      </div>
